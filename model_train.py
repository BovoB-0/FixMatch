...
def save_checkpoint(state: dict, is_best: bool, checkpoint: str, filename: str = 'checkpoint.pth.tar') -> None:
...
def main() -> None:
    ...
    parser.add_argument('--data_cifar', default='cifar10', type=str, choices=['cifar10', 'cifar100'], help='data_cifar name')
    parser.add_argument('--num-labeled', type=int, default=4000, help='number of labeled data')
    parser.add_argument("--expand-labels", action="store_true", help="expand labels to fit eval steps")
    parser.add_argument('--arch', default='wideresnet', type=str, choices=['wideresnet', 'resnext'], help='data_cifar name')
    ...
    logger.warning(f"Process rank: {args.local_rank}, device: {args.device}, n_gpu: {args.n_gpu}, distributed training: {bool(args.local_rank != -1)}, 16-bits training: {args.amp}")
...
def train(args, labeled_trainloader: DataLoader, unlabeled_trainloader: DataLoader, test_loader: DataLoader,
          model: torch.nn.Module, optimizer: optim.Optimizer, ema_model: torch.nn.Module, scheduler: LambdaLR) -> None:
    if args.amp:
        from apex import amp
    ...
    def reset_iterator(loader: DataLoader, epoch: int, world_size: int) -> iter:
        if world_size > 1:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            try:
                ...
                labeled_epoch += 1
                labeled_iter = reset_iterator(labeled_trainloader, labeled_epoch, args.world_size)
                inputs_x, targets_x = next(labeled_iter)
            ...
            try:
                ...
                unlabeled_epoch += 1
                unlabeled_iter = reset_iterator(unlabeled_trainloader, unlabeled_epoch, args.world_size)
                (inputs_u_w, inputs_u_s), _ = next(unlabeled_iter)
            ...
            if not args.no_progress:
                p_bar.set_description(
                    ...
                    f"Loss_x: {losses_x.avg:.4f}. Loss_u: {losses_u.avg:.4f}. Mask: {mask_probs.avg:.2f}.")
                p_bar.update()
...
def test(args, test_loader: DataLoader, model: torch.nn.Module, epoch: int) -> Tuple[float, float]:
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
